<div class="bsx-mobile-fixed-wrapper d-md-none position-sticky bg-info header-shadow bottom-0">
  <?php
    ...
    wp_nav_menu( 
      array( 
        ...
        'add_title_content' => '<div class="bsx-mobile-fixed-nav-icon pt-2 pb-1"><span class="fas fa-%description$s fa-lg" aria-hidden="true"></span></div><div class="bsx-mobile-fixed-nav-title small pb-2">%title$s</div>' // custom filter add_additional_content_in_a(), see functions.php (use %description$s to insert description, use %title$s to insert title into html)
      ) 
    );
  ?>
</div>